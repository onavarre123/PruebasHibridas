...
function InitBaseDatos() {
    //TBL_VERSION_ENVIO
    BDActualizacion("CREATE TABLE IF NOT EXISTS TBL_VERSION_ENVIO " +
                            "(FECUPDATE DATETIME null)");

    //TBL_VERSION_RECEPCION
    BDActualizacion("CREATE TABLE IF NOT EXISTS TBL_VERSION_RECEPCION " +
                            "(FECUPDATE DATETIME null)");
...
function firstSync(ID_USUARIO, RESPUESTA) {
    //FECHA ACTUALIZACION DE APP-SERVER
    BDActualizacion("DELETE FROM TBL_VERSION_ENVIO");
    BDActualizacion("INSERT INTO TBL_VERSION_ENVIO VALUES('" + fecActual() + "');");
    BDActualizacion("DELETE FROM TBL_VERSION_RECEPCION");
    BDActualizacion("INSERT INTO TBL_VERSION_RECEPCION VALUES('" + fecActual() + "');");
    ...
    if ($("#ACEPTADISPOSE").val() == "on") {
        ...
        objetoServidor($("#URL_SINCRONIZAR").val() + '/Sync/ClientesCargaInicial?ID_USUARIO=' + ID_USUARIO, function (data) {
            var obj = jQuery.parseJSON(data);
            $.each(obj, function (i, valor) {
                BDActualizacion("INSERT INTO TBL_CLIENTE VALUES(" + valor.ID + ",'" + valor.CODINTERNO + "','" + valor.NOMBRE + "','" + valor.ESTADO + "','" + valor.DIRFACTURA + "','" + valor.DIRDESTINO + "','" + valor.TELF1 + "','" + valor.TELF2 + "','" + valor.LATITUD + "','" + valor.LONGITUD + "','" + valor.IMAGEN + "','" + valor.COMENTARIO + "','" + fecActual() + "')");
            });
        });
    }
